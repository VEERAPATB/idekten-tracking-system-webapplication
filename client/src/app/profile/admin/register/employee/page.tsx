...
const AddEmployee = () => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const response = await axiosAuth.post('/api/register/employee', {
        ...userData,
        cellPhoneNumber: defaultPhoneNumber,
        citizenID: defaultCitizenId,
        birthDate: defaultBirthDate
      })
      router.push(`/profile/admin/register/employee`);
    } catch (error) {
  ...
  const fetchPreparedData = async () => {
    setLoading(true)
    try {
      const data = await axiosAuth.get("/api/register/employee");
      setPreparedData({
  ...
  useEffect(() => {
    ...
    var forth = Boolean(
      userData.bankName &&
      userData.bankNumber && (userData.bankNumber.length >= 10) &&
      userData.branch
    )
  ...
  return (
    <div className=" fixed mt-[5rem] left-1/2 -translate-x-1/2">
      ...
      <form
        className="relative rounded-[0.625rem] h-[42.25rem] w-[22rem] md:w-[42rem] bg-white shadow-[10px_10px_10px_0px_rgba(0,0,0,0.25)]"
              ...
              >
                <div className="relative w-full information flex flex-col gap-[2.5rem]">
                  ...
                  <InputText
                    ...
                    onChange={(e: ChangeEvent<HTMLInputElement>) =>
                      setUserData({ ...userData, cellPhoneNumber: e.target.value })
                    }
                  />
                </div>
              </div>
              ...
              >
                <div className="relative w-full information flex flex-col gap-[2.5rem]">
                  ...
                  <InputText
                    ...
                    type="text"
                    onChange={(e: ChangeEvent<HTMLInputElement>) =>
                      setUserData({ ...userData, bankNumber: e.target.value })
                    }
