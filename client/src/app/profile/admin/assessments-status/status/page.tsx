...
const page = (props: PageProps) => {
  ...
  const [page, setPage] = useState(
    Number(props.searchParams?.p) ? Number(props.searchParams?.p) : 1
  );
  ...
  return (
    <>
      <div className=" container mx-auto mt-[4.5rem] flex flex-col gap-[0.75rem]">
        <div className="w-[10rem] ml-[1rem] md:ml-0">
          <Button
            label="ย้อนกลับ"
            border={true}
                ...
                list={[
                  {
                    ...
                    fn: () => {
                      ...
                      setStatus('all')
                      setPage(1)
                      router.push(`${pathname}?approve=all&class=${formInfo.class}&id=${formInfo.id}&hp=${previousPage}&${props.searchParams?.s && `s=${String(props.searchParams?.s)}`}`);
                    },
                  },
                  {
                    ...
                    fn: () => {
                      ...
                      setStatus('no')
                      setPage(1)
                      router.push(`${pathname}?approve=no&class=${formInfo.class}&id=${formInfo.id}&hp=${previousPage}&${props.searchParams?.s && `s=${String(props.searchParams?.s)}`}`);
                    },
                  },
                  {
                    name: "ประเมินแล้ว",
                    ...
                    fn: () => {
                      ...
                      setStatus('yes')
                      setPage(1)
                      router.push(`${pathname}?approve=yes&class=${formInfo.class}&id=${formInfo.id}&hp=${previousPage}&${props.searchParams?.s && `s=${String(props.searchParams?.s)}`}`);
                    },
                  },
                ]}
              />
