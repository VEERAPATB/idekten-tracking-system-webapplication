...
const page = (props: PageProps) => {
  ...
  const previousPage = String(props.searchParams?.hp)
  ...
  return (
    <>
      {popupInfo && (
        <>
          <BlurBack open />
          <div className="fixed top-1/2 left-1/2 mx-auto -translate-x-1/2 -translate-y-1/2 z-[100]">
            <PopupDetail
              ...
              component={
                <div className="">
                  <div className="grid grid-cols-2 gap-[0.875rem] mb-[1rem] md:mb-0">
                    <p className=" font-medium">ชื่อ-นามสกุล <br className=" md:hidden" /> (นักเรียน) :</p>
                    <p>{approveEach?.student.name}</p>
                    <p className=" font-medium">
                      ชื่อ-นามสกุล <br className=" md:hidden" /> (ผู้ประเมิน) :
                    </p>
                    <p>{approveEach?.employee.name}</p>
                    <p className=" row-span-1 md:row-span-2 font-medium md:text-left">
                      ช่องทางการติดต่อ <br className=" md:hidden" />{" "}
                      (ผู้ประเมิน) :
                    </p>
                    <p>{approveEach?.employee.telephone}</p>
                    <p className=" text-right col-span-2 md:col-span-1 md:text-left">{approveEach?.employee.email}</p>
                  </div>
      ...
      {popupApprove && (
        <>
          ...
          <div className="mx-[1.5rem] fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-[100]">
            <PopupDanger
              head="โปรดตรวจสอบผลลัพธ์การประเมิน"
              description={`"${approveEach?.feedback}"`}
              onClose={() => setPopupApprove(false)}
              function={{
                name: "ยินยัน",
                function: () => {
                  performApproveEach(approveEach?.id, newFeedback);
      ...
      <div className=" container mx-auto mt-[4.5rem] flex flex-col gap-[0.75rem]">
        ...
        <div className="w-[10rem]">
          <Button
            ...
            onClick={() => {
              setLoading(true);
              router.replace(`${previousPage}&${props.searchParams?.s && `status=${String(props.searchParams?.s)}`}`)
            }}
          />
        </div>
        <div className="container mx-auto bg-white w-full shadow-[10px_10px_10px_0_rgba(0,0,0,0.25)] rounded-[0.625rem] relative">
          <div className="pt-[4rem] md:mx-14 relative mx-[1.5rem]">
                ...
                list={[
                  {
                    ...
                    fn: () => {
                      ...
                      setStatus("no");
                      router.push(
                        `${pathname}?status=no&id=${formInfo.id}&class=${formInfo.class}&hp=${previousPage}&${props.searchParams?.s && `s=${String(props.searchParams?.s)}`}`
                      );
                    },
                  },
                  {
                    ...
                    fn: () => {
                      ...
                      setStatus("yes");
                      router.push(
                        `${pathname}?status=yes&id=${formInfo.id}&class=${formInfo.class}&hp=${previousPage}&${props.searchParams?.s && `s=${String(props.searchParams?.s)}`}`
                      );
                    },
                  },
                ]}
              />
              <div className="md:w-[20rem] hidden">
                <SearchBar
                  ...
                  <Button
                    ...
                    style={`${
                      data && data?.meta.total < 1
                        ? "bg-[#9B9B9B]"
                        : "bg-[#D29F48]"
                    } text-[#F3F3F3] px-[1rem] py-[0.5rem]`}
                    onClick={() => setDanger(true)}
                  />
                </div>
              </div>
            )}
            <div className=" text-[#9B9B9B] text-right text-sm mt-[1rem]">
