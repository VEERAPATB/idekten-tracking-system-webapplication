      ...
      >
        <table className={`w-full ${fadeIn ? "opacity-100" : "opacity-0"}`}>
          <thead className="bg-[#73408A] border-b-2 border-[#73408A]">
            <tr>
              <th className=" w-4/12 md:w-2/12 p-3 text-sm font-semibold tracking-wide text-left text-white">
                Week
              </th>
              <th className=" w-6 p-3 text-sm font-semibold tracking-wide text-left text-white sm:hidden">
                Avg.score
              </th>
              <th className="w-1 p-3 text-sm font-semibold tracking-wide text-left text-white sm:hidden"></th>
              {Data.Topics.map((item, index) => (
                <th className=" w-5 p-1 text-sm font-semibold tracking-wide text-left text-white hidden sm:table-cell">
                  {Data.Topics[index]}
                </th>
              ))}
              <th className="w-2/6 p-3 text-sm font-semibold tracking-wide text-left text-white hidden sm:table-cell">
                Feedback
              </th>
          ...
          <tbody className="divide-y divide-[#73408A]">
            {Data.Forms.map((item, index) => (
              <tr key={index} className="bg-white">
                <td className="p-3 md:text-base text-black ">{item.week}</td>
                {valueTopicAVG(item)}
                <td className="p-3 text-base text-black  sm:hidden"></td>
                {valueTopic(item)}
                <td className="p-3 text-base text-black  hidden sm:table-cell ">
                  {item.Feedback.substring(0, 40)} ...
                </td>
                <td className=" w-12 h-12 p-3 text-base text-black  ">
                  <button
                    onClick={() => {
                      setDisplay(true);
                      setDataDisplay({ ...item });
                    }}
                    className="bg-[#73408A] rounded-xl w-12 h-12 p-1 flex items-center justify-center m-3"
                  >
                    <Image
                      alt=""
